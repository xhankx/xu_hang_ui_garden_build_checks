...
`)}
`).replace("<<type>>",isTypescript?": StorybookConfig":"").replace("<<mainContents>>",mainContents),prettier=(await import("prettier")).default,mainPath=`./${storybookConfigFolder}/main.${isTypescript?"ts":"js"}`,prettyMain=prettier.format((0,import_ts_dedent.dedent)(mainJsContents),{...prettier.resolveConfig.sync(process.cwd()),filepath:mainPath});await import_fs_extra3.default.writeFile(mainPath,prettyMain,{encoding:"utf8"})}async function configurePreview(options){let{prefix:frameworkPrefix=""}=options.frameworkPreviewParts||{},isTypescript=options.language==="typescript-4-9"||options.language==="typescript-3-8",rendererPackage=options.rendererId&&!externalFrameworks.map(({name})=>name).includes(options.rendererId)?`@storybook/${options.rendererId}`:null,previewPath=`./${options.storybookConfigFolder}/preview.${isTypescript?"ts":"js"}`;if(await import_fs_extra3.default.pathExists(previewPath))return;let prefix=[isTypescript&&rendererPackage?`import type { Preview } from '${rendererPackage}'`:"",frameworkPrefix].filter(Boolean).join(`
...
`.trimStart(),promptForCompoDocs=async()=>{let{useCompoDoc}=await(0,import_prompts4.default)({type:"confirm",name:"useCompoDoc",message:"Do you want to use Compodoc for documentation?"});return useCompoDoc},AngularJSON=class{constructor(){if(!import_fs4.default.existsSync(ANGULAR_JSON_PATH))throw new Error("An angular.json file was not found in the current working directory. Storybook needs it to work properly, so please rerun the command at the root of your project, where the angular.json file is located. More info: https://storybook.js.org/docs/angular/faq#error-no-angularjson-file-found");let jsonContent=import_fs4.default.readFileSync(ANGULAR_JSON_PATH,"utf8");this.json=JSON.parse(jsonContent)}get projects(){return this.json.projects}get projectsWithoutStorybook(){return Object.keys(this.projects).filter(projectName=>{let{architect}=this.projects[projectName];return!architect.storybook})}get hasStorybookBuilder(){return Object.keys(this.projects).some(projectName=>{let{architect}=this.projects[projectName];return Object.keys(architect).some(key=>architect[key].builder==="@storybook/angular:start-storybook")})}get rootProject(){let rootProjectName=Object.keys(this.projects).find(projectName=>{let{root}=this.projects[projectName];return root===""||root==="."});return rootProjectName?this.projects[rootProjectName]:null}getProjectSettingsByName(projectName){return this.projects[projectName]}async getProjectName(){if(this.projectsWithoutStorybook.length>1){let{projectName}=await(0,import_prompts4.default)({type:"select",name:"projectName",message:"For which project do you want to generate Storybook configuration?",choices:this.projectsWithoutStorybook.map(name=>({title:name,value:name}))});return projectName}return this.projectsWithoutStorybook[0]}addStorybookEntries({angularProjectName,storybookFolder,useCompodoc,root}){let{architect}=this.projects[angularProjectName],baseOptions={configDir:storybookFolder,browserTarget:`${angularProjectName}:build`,compodoc:useCompodoc,...useCompodoc&&{compodocArgs:["-e","json","-d",root||"."]}};architect.storybook||(architect.storybook={builder:"@storybook/angular:start-storybook",options:{...baseOptions,port:6006}}),architect["build-storybook"]||(architect["build-storybook"]={builder:"@storybook/angular:build-storybook",options:{...baseOptions,outputDir:Object.keys(this.projects).length===1?"storybook-static":`dist/storybook/${angularProjectName}`}})}write(){import_fs4.default.writeFileSync(ANGULAR_JSON_PATH,JSON.stringify(this.json,null,2))}};var generator=async(packageManager,npmOptions,options,commandOptions)=>{var _a,_b;let angularVersionFromDependencies=(_a=import_semver5.default.coerce((await packageManager.retrievePackageJson()).dependencies["@angular/core"]))==null?void 0:_a.version,angularVersionFromDevDependencies=(_b=import_semver5.default.coerce((await packageManager.retrievePackageJson()).devDependencies["@angular/core"]))==null?void 0:_b.version,angularVersion=angularVersionFromDependencies||angularVersionFromDevDependencies,updatedOptions=import_semver5.default.gte(angularVersion,"12.0.0")?{...options,builder:"webpack5"}:options,angularJSON=new AngularJSON;if(!angularJSON.projects||angularJSON.projects&&Object.keys(angularJSON.projects).length===0)throw new Error("Storybook was not able to find any projects in your angular.json file. Are you sure this is an Angular CLI project?");if(angularJSON.projectsWithoutStorybook.length===0)throw new Error("Every project in your workspace is already set up with Storybook. There is nothing to do!");let angularProjectName=await angularJSON.getProjectName();paddedLog(`Adding Storybook support to your "${angularProjectName}" project`);let angularProject=angularJSON.getProjectSettingsByName(angularProjectName);if(!angularProject)throw new Error(`Somehow we were not able to retrieve the "${angularProjectName}" project in your angular.json file. This is likely a bug in Storybook, please file an issue.`);let{root,projectType}=angularProject,{projects}=angularJSON,useCompodoc=commandOptions.yes?!0:await promptForCompoDocs(),storybookFolder=root?`${root}/.storybook`:".storybook";angularJSON.addStorybookEntries({angularProjectName,storybookFolder,useCompodoc,root}),angularJSON.write(),await baseGenerator(packageManager,npmOptions,{...updatedOptions,...useCompodoc&&{frameworkPreviewParts:{prefix:compoDocPreviewPrefix}}},"angular",{...useCompodoc&&{extraPackages:["@compodoc/compodoc"]},addScripts:!1,componentsDestinationPath:root?`${root}/src/stories`:void 0,storybookConfigFolder:storybookFolder},"angular"),Object.keys(projects).length===1&&packageManager.addScripts({storybook:`ng run ${angularProjectName}:storybook`,"build-storybook":`ng run ${angularProjectName}:build-storybook`});let projectTypeValue=projectType||"application";projectTypeValue!=="application"&&projectTypeValue!=="library"&&(projectTypeValue="application");let templateDir=(0,import_path9.join)(getCliDir(),"templates","angular",projectTypeValue);return templateDir&&copyTemplate(templateDir,root||void 0),{projectName:angularProjectName,configDir:storybookFolder}},ANGULAR_default=generator;var generator2=async(packageManager,npmOptions,options)=>{await baseGenerator(packageManager,npmOptions,options,"ember",{extraPackages:["babel-plugin-ember-modules-api-polyfill","babel-plugin-htmlbars-inline-precompile"],staticDir:"dist"})},EMBER_default=generator2;var generator3=async(packageManager,npmOptions,options)=>{let extraPackages=detectLanguage()==="javascript"?["prop-types"]:[];await baseGenerator(packageManager,npmOptions,options,"react",{extraPackages})},REACT_default=generator3;var generator4=async(packageManager,npmOptions)=>{let packageJson=await packageManager.retrievePackageJson(),missingReactDom=!packageJson.dependencies["react-dom"]&&!packageJson.devDependencies["react-dom"],reactVersion=packageJson.dependencies.react,packagesToResolve=["react-native-safe-area-context","@react-native-async-storage/async-storage","@react-native-community/datetimepicker","@react-native-community/slider","@storybook/addon-ondevice-controls","@storybook/addon-ondevice-actions","@storybook/react-native"],packagesWithFixedVersion=["@storybook/addon-actions@^6.5.16","@storybook/addon-controls@^6.5.16"],resolvedPackages=await packageManager.getVersionedPackages(packagesToResolve),packages=[...await getBabelDependencies(packageManager,packageJson),...packagesWithFixedVersion,...resolvedPackages,missingReactDom&&reactVersion&&`react-dom@${reactVersion}`].filter(Boolean);await packageManager.addDependencies({...npmOptions,packageJson},packages),packageManager.addScripts({"storybook-generate":"sb-rn-get-stories","storybook-watch":"sb-rn-watcher"});let storybookConfigFolder=".storybook";await copyTemplateFiles({packageManager,renderer:"react-native",language:"javascript",destination:storybookConfigFolder,includeCommonAssets:!1})},REACT_NATIVE_default=generator4;var import_path10=__toESM(require("path")),import_fs5=__toESM(require("fs")),import_semver6=__toESM(require("semver")),import_ts_dedent10=__toESM(require("ts-dedent"));var generator5=async(packageManager,npmOptions,options)=>{var _a;let monorepoRootPath=import_path10.default.join(__dirname,"..","..","..","..","..",".."),extraMain=options.linkable?{webpackFinal:`%%(config) => {
    ...
    %%`}:{},packageJson=await packageManager.retrievePackageJson(),craVersion=(_a=import_semver6.default.coerce(packageJson.dependencies["react-scripts"]))==null?void 0:_a.version,isCra5OrHigher=craVersion&&import_semver6.default.gte(craVersion,"5.0.0"),updatedOptions=isCra5OrHigher?{...options,builder:"webpack5"}:options,extraPackages=[];isCra5OrHigher&&(extraPackages.push("webpack"),extraPackages.push("babel-plugin-named-exports-order"),extraPackages.push("prop-types"));let extraAddons=[`@storybook/preset-create-react-app@${versions_default["@storybook/preset-create-react-app"]}`];if(!isCra5OrHigher)throw new Error(import_ts_dedent10.default`
...
`),installableProjectTypes.sort().forEach(framework=>paddedLog(`- ${framework}`)),logger7.log(),new HandledError(`Unknown project type supplied: ${projectTypeProvided}`);else try{projectType=detect(packageJson,options)}catch(err){throw done(err.message),new HandledError(err)}done();let storybookInstantiated=isStorybookInstantiated();if(options.force===!1&&storybookInstantiated&&projectType!=="ANGULAR"){logger7.log();let{force}=await(0,import_prompts5.default)([{type:"confirm",name:"force",message:"We found a .storybook config directory in your project. Therefore we assume that Storybook is already instantiated for your project. Do you still want to continue and force the initialization?"}]);logger7.log(),force?options.force=!0:process.exit(0)}let installResult=await installStorybook(projectType,packageManager,options);options.skipInstall||await packageManager.installDependencies(),options.disableTelemetry||(0,import_telemetry.telemetry)("init",{projectType}),logger7.log(`
For more information visit:`,import_chalk4.default.cyan("https://storybook.js.org")),projectType==="ANGULAR"?(logger7.log(`
To run your Storybook, type:
`),codeLog([`ng run ${installResult.projectName}:storybook`])):projectType==="REACT_NATIVE"?(logger7.log(),logger7.log(import_chalk4.default.yellow(`NOTE: installation is not 100% automated.
...
`),logger7.log(import_chalk4.default.cyan("https://github.com/storybookjs/react-native")),logger7.log()):(logger7.log(`
To run your Storybook, type:
`),codeLog([packageManager.getRunStorybookCommand()])),logger7.log()}async function initiate(options,pkg2){await(0,import_core_server.withTelemetry)("init",{cliOptions:options,printError:err=>!err.handled&&logger7.error(err)},()=>doInitiate(options,pkg2))}var import_path11=__toESM(require("path")),import_fs6=__toESM(require("fs")),import_cross_spawn2=require("cross-spawn"),import_core_common=require("@storybook/core-common"),import_csf_tools2=require("@storybook/csf-tools");var logger8=console,LEGACY_CONFIGS=["addons","config","presets"],postinstallAddon=async(addonName,isOfficialAddon)=>{let skipMsg=null;isOfficialAddon?import_fs6.default.existsSync(".storybook")?(skipMsg="no codmods found",LEGACY_CONFIGS.forEach(config=>{try{let codemod=require.resolve(`${getPackageName(addonName,isOfficialAddon)}/postinstall/${config}.js`);commandLog(`Running postinstall script for ${addonName}`)();let configFile=import_path11.default.join(".storybook",`${config}.ts`);import_fs6.default.existsSync(configFile)||(configFile=import_path11.default.join(".storybook",`${config}.js`),import_fs6.default.existsSync(configFile)||import_fs6.default.writeFileSync(configFile,"","utf8")),(0,import_cross_spawn2.sync)("npx",["jscodeshift","-t",codemod,configFile],{stdio:"inherit",shell:!0}),skipMsg=null}catch{}})):skipMsg="no .storybook config":skipMsg="unofficial addon",skipMsg&&commandLog(`Skipping postinstall for ${addonName}, ${skipMsg}`)()},getVersionSpecifier=addon=>{let groups=/^(...*)@(.*)$/.exec(addon);return groups?[groups[1],groups[2]]:[addon,void 0]};async function add(addon,options){let{packageManager:pkgMgr}=options;options.useNpm&&(useNpmWarning(),pkgMgr="npm");let packageManager=JsPackageManagerFactory.getPackageManager({force:pkgMgr}),packageJson=await packageManager.retrievePackageJson(),[addonName,versionSpecifier]=getVersionSpecifier(addon),{mainConfig,version:storybookVersion}=(0,import_core_common.getStorybookInfo)(packageJson);if(!mainConfig){logger8.error("Unable to find storybook main.js config");return}let main=await(0,import_csf_tools2.readConfig)(mainConfig);logger8.log(`Verifying ${addonName}`);let latestVersion=await packageManager.latestVersion(addonName);latestVersion||logger8.error(`Unknown addon ${addonName}`);let isStorybookAddon=addonName.startsWith("@storybook/"),addonWithVersion=`${addonName}@${versionSpecifier||(isStorybookAddon?storybookVersion:latestVersion)}`;logger8.log(`Installing ${addonWithVersion}`),await packageManager.addDependencies({installAsDevDependencies:!0},[addonWithVersion]),logger8.log(`Adding '${addon}' to main.js addons field.`),main.appendValueToArray(["addons"],addonName),await(0,import_csf_tools2.writeConfig)(main),options.skipPostinstall||await postinstallAddon(addon,isStorybookAddon)}var import_codemod=require("@storybook/codemod");var import_prompts8=__toESM(require("prompts")),import_chalk26=__toESM(require("chalk")),import_boxen2=__toESM(require("boxen")),import_fs_extra12=require("fs-extra"),import_tempy2=__toESM(require("tempy")),import_ts_dedent34=__toESM(require("ts-dedent")),import_path15=require("path"),import_core_common9=require("@storybook/core-common"),import_semver23=__toESM(require("semver"));var import_chalk8=__toESM(require("chalk")),import_ts_dedent15=require("ts-dedent"),import_semver10=__toESM(require("semver"));var import_chalk7=__toESM(require("chalk")),import_ts_dedent14=require("ts-dedent"),import_semver9=__toESM(require("semver"));var import_chalk6=__toESM(require("chalk")),import_semver8=__toESM(require("semver")),import_ts_dedent13=__toESM(require("ts-dedent"));var import_core_common2=require("@storybook/core-common"),import_csf_tools3=require("@storybook/csf-tools"),import_chalk5=__toESM(require("chalk")),import_semver7=__toESM(require("semver")),import_ts_dedent12=__toESM(require("ts-dedent")),logger9=console,getStorybookData=async({packageManager,configDir:userDefinedConfigDir})=>{var _a;let packageJson=await packageManager.retrievePackageJson(),{mainConfig:mainConfigPath,version:storybookVersionSpecifier,configDir:configDirFromScript,previewConfig:previewConfigPath}=(0,import_core_common2.getStorybookInfo)(packageJson,userDefinedConfigDir),storybookVersion=storybookVersionSpecifier&&((_a=import_semver7.default.coerce(storybookVersionSpecifier))==null?void 0:_a.version),configDir=userDefinedConfigDir||configDirFromScript||".storybook",mainConfig;try{mainConfig=await(0,import_core_common2.loadMainConfig)({configDir,noCache:!0})}catch(err){throw new Error(import_ts_dedent12.default`Unable to find or evaluate ${import_chalk5.default.blue(mainConfigPath)}: ${err.message}`)}return{configDir,mainConfig,storybookVersionSpecifier,storybookVersion,mainConfigPath,previewConfigPath}},updateMainConfig=async({mainConfigPath,dryRun},callback)=>{try{let main=await(0,import_csf_tools3.readConfig)(mainConfigPath);await callback(main),dryRun||await(0,import_csf_tools3.writeConfig)(main)}catch(e){logger9.info(`\u274C The migration failed to update your ${import_chalk5.default.blue(mainConfigPath)} on your behalf because of the following error:
        ${e}
`),logger9.info(`\u26A0\uFE0F Storybook automigrations are based on AST parsing and it's possible that your ${import_chalk5.default.blue(mainConfigPath)} file contains a non-standard format (e.g. your export is not an object) or that there was an error when parsing dynamic values (e.g. "require" calls, or usage of environment variables). When your main config is non-standard, automigrations are unfortunately not possible. Please follow the instructions given previously and follow the documentation to make the updates manually.`)}},getAddonNames=mainConfig=>(mainConfig.addons||[]).map(addon=>{let name="";if(typeof addon=="string"?name=addon:typeof addon=="object"&&(name=addon.name),!name.startsWith("."))return name.replace(/\/dist\/.*/,"").replace(/\.[mc]?[tj]?s[x]?$/,"").replace(/\/register$/,"").replace(/\/manager$/,"").replace(/\/preset$/,"")}).filter(Boolean);var logger10=console,checkWebpack5Builder=async({configDir,packageManager})=>{var _a;let{mainConfig,storybookVersion}=await getStorybookData({configDir,packageManager});if(import_semver8.default.lt(storybookVersion,"6.3.0"))return logger10.warn(import_ts_dedent13.default`
      ...
      `.trim()),null;if(import_semver8.default.gte(storybookVersion,"7.0.0"))return null;if(!mainConfig)return logger10.warn("Unable to find storybook main.js config"),null;let builder=(_a=mainConfig.core)==null?void 0:_a.builder;return builder&&builder!=="webpack4"?(logger10.info(`Found builder ${builder}, skipping`),null):{storybookVersion}};var logger11=console,webpack5={id:"webpack5",async check({configDir,packageManager}){var _a;let webpackVersion=(await packageManager.retrievePackageJson()).dependencies.webpack,webpackCoerced=(_a=import_semver9.default.coerce(webpackVersion))==null?void 0:_a.version;if(!webpackCoerced||import_semver9.default.lt(webpackCoerced,"5.0.0")||import_semver9.default.gte(webpackCoerced,"6.0.0"))return null;let builderInfo=await checkWebpack5Builder({configDir,packageManager});return builderInfo?{webpackVersion,...builderInfo}:null},prompt({webpackVersion}){let webpackFormatted=import_chalk7.default.cyan(`webpack ${webpackVersion}`);return import_ts_dedent14.dedent`
    ...
    `},async run({result:{storybookVersion,webpackVersion},packageManager,dryRun,mainConfigPath}){let deps=[`@storybook/builder-webpack5@${storybookVersion}`];webpackVersion||deps.push("webpack@5"),logger11.info(`\u2705 Adding dependencies: ${deps}`),dryRun||await packageManager.addDependencies({installAsDevDependencies:!0},deps),logger11.info("\u2705 Setting `core.builder` to `@storybook/builder-webpack5` in main.js"),dryRun||await updateMainConfig({mainConfigPath,dryRun},async main=>{main.setFieldValue(["core","builder"],"@storybook/builder-webpack5")})}};var cra5={id:"cra5",async check({packageManager,configDir}){var _a;let craVersion=(await packageManager.getAllDependencies())["react-scripts"],craCoerced=(_a=import_semver10.default.coerce(craVersion))==null?void 0:_a.version;if(!craCoerced||import_semver10.default.lt(craCoerced,"5.0.0"))return null;let builderInfo=await checkWebpack5Builder({configDir,packageManager});return builderInfo?{craVersion,...builderInfo}:null},prompt({craVersion}){let craFormatted=import_chalk8.default.cyan(`Create React App (CRA) ${craVersion}`);return import_ts_dedent15.dedent`
    ...
    `},async run(options){return webpack5.run({...options,result:{webpackVersion:null,...options.result}})}};var import_chalk9=__toESM(require("chalk")),import_ts_dedent16=require("ts-dedent"),import_semver11=__toESM(require("semver"));var angular12={id:"angular12",async check({packageManager,configDir}){var _a;let angularVersion=(await packageManager.getAllDependencies())["@angular/core"],angularCoerced=(_a=import_semver11.default.coerce(angularVersion))==null?void 0:_a.version;if(!angularCoerced||import_semver11.default.lt(angularCoerced,"12.0.0"))return null;let builderInfo=await checkWebpack5Builder({packageManager,configDir});return builderInfo?{angularVersion,...builderInfo}:null},prompt({angularVersion}){let angularFormatted=import_chalk9.default.cyan(`Angular ${angularVersion}`);return import_ts_dedent16.dedent`
      We've detected you are running ${angularFormatted} which is powered by webpack5.
      Your Storybook's main.js files specifies webpack4, which is incompatible.

      In order to work with your version of Angular, we need to install Storybook's ${import_chalk9.default.cyan("@storybook/builder-webpack5")}.

      More info: ${import_chalk9.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#angular12-upgrade")}
    `},async run(options){return webpack5.run({...options,result:{webpackVersion:null,...options.result}})}};var import_chalk10=__toESM(require("chalk")),import_ts_dedent17=require("ts-dedent"),import_semver12=__toESM(require("semver"));var vue3={id:"vue3",async check({configDir,packageManager}){var _a;let vueVersion=(await packageManager.getAllDependencies()).vue,vueCoerced=(_a=import_semver12.default.coerce(vueVersion))==null?void 0:_a.version;if(!vueCoerced||import_semver12.default.lt(vueCoerced,"3.0.0"))return null;let builderInfo=await checkWebpack5Builder({configDir,packageManager});return builderInfo?{vueVersion,...builderInfo}:null},prompt({vueVersion,storybookVersion}){let vueFormatted=import_chalk10.default.cyan(`Vue ${vueVersion}`),sbFormatted=import_chalk10.default.cyan(`Storybook ${storybookVersion}`);return import_ts_dedent17.dedent`
    ...
    `},async run(options){return webpack5.run({...options,result:{webpackVersion:null,...options.result}})}};var import_ts_dedent18=require("ts-dedent"),import_semver13=__toESM(require("semver")),import_path12=require("path");function slash(path14){return/^\\\\\?\\/.test(path14)?path14:path14.replace(/\\/g,"/")}var import_globby=__toESM(require("globby")),import_core_common3=require("@storybook/core-common");var logger12=console,mdxgfm={id:"github-flavored-markdown-mdx",async check({configDir,packageManager}){var _a,_b,_c;let{mainConfig,storybookVersion}=await getStorybookData({packageManager,configDir});if(!import_semver13.default.gte(storybookVersion,"7.0.0"))return null;let hasMDXFiles=await((_a=mainConfig==null?void 0:mainConfig.stories)==null?void 0:_a.reduce(async(acc,item)=>{if(await acc===!0)return!0;let pattern=typeof item=="string"?slash((0,import_path12.join)(configDir,item)):slash((0,import_path12.join)(configDir,item.directory,item.files));return(await(0,import_globby.default)(pattern,(0,import_core_common3.commonGlobOptions)(pattern))).some(f=>f.endsWith(".mdx"))},Promise.resolve(!1)));return((_b=mainConfig==null?void 0:mainConfig.features)==null?void 0:_b.legacyMdx1)===!0||!1||!hasMDXFiles||!!((_c=mainConfig.addons)!=null&&_c.find(item=>{var _a2,_b2,_c2,_d;return item==="@storybook/addon-mdx-gfm"?!0:typeof item=="string"?!1:item.name==="@storybook/addon-docs"?((_d=(_c2=(_b2=(_a2=item.options)==null?void 0:_a2.mdxPluginOptions)==null?void 0:_b2.mdxCompileOptions)==null?void 0:_c2.remarkPlugins)==null?void 0:_d.length)>0:!1}))?null:{}},prompt(){return import_ts_dedent18.dedent`
    ...
    `},async run({result:{eslintFile,unsupportedExtension},packageManager,dryRun,skipInstall}){let deps=["eslint-plugin-storybook"];if(logger13.info(`\u2705 Adding dependencies: ${deps}`),dryRun||await packageManager.addDependencies({installAsDevDependencies:!0,skipInstall},deps),!dryRun&&unsupportedExtension){logger13.info(import_ts_dedent19.dedent`
      ...
      `);return}dryRun||await configureEslintPlugin(eslintFile,packageManager)}};var import_chalk12=__toESM(require("chalk")),import_ts_dedent20=require("ts-dedent"),import_csf_tools4=require("@storybook/csf-tools");var logger14=console,builderVite={id:"builder-vite",async check({configDir,packageManager}){var _a;let packageJson=await packageManager.retrievePackageJson(),{mainConfig}=await getStorybookData({configDir,packageManager}),builder=(_a=mainConfig.core)==null?void 0:_a.builder;return(typeof builder=="string"?builder:builder==null?void 0:builder.name)!=="storybook-builder-vite"?null:{builder,packageJson}},prompt({builder}){let builderFormatted=import_chalk12.default.cyan(JSON.stringify(builder,null,2));return import_ts_dedent20.dedent`
...
`:""}
      ...
      `},async run({result:{packageJson,hasSbBinary,hasStorybookBinary},packageManager,dryRun,skipInstall}){if(hasSbBinary&&(logger16.info("\u2705 Removing 'sb' dependency"),dryRun||await packageManager.removeDependencies({skipInstall:skipInstall||!hasStorybookBinary,packageJson},["sb"])),!hasStorybookBinary&&(logger16.log(),logger16.info("\u2705 Adding 'storybook' as dev dependency"),logger16.log(),!dryRun)){let versionToInstall=getStorybookVersionSpecifier(packageJson);await packageManager.addDependencies({installAsDevDependencies:!0,packageJson,skipInstall},[`storybook@${versionToInstall}`])}}};var import_chalk15=__toESM(require("chalk")),import_ts_dedent23=__toESM(require("ts-dedent")),import_find_up4=__toESM(require("find-up")),import_semver16=__toESM(require("semver")),import_core_common6=require("@storybook/core-common");var import_core_common4=require("@storybook/core-common"),import_prompts6=__toESM(require("prompts")),detectRenderer=async packageJson=>{let allDependencies={...packageJson.dependencies,...packageJson.devDependencies,...packageJson.peerDependencies},matches=Object.keys(import_core_common4.rendererPackages).filter(pkg2=>!!allDependencies[pkg2]),[rendererPackage]=matches;return matches.length>1&&(rendererPackage=(await(0,import_prompts6.default)([{type:"select",name:"rendererPackage",hint:"> - Use arrow-keys. Return to submit. To automate this next time, you can pass the renderer to the CLI via the --renderer flag",message:"Tried to detect a Storybook renderer in your project but found multiple. This could happen in monorepos, when projects contain multiple Storybook packages in package.json. Please select the correct one:",choices:matches.map(type=>({title:type,value:type}))}])).rendererPackage),rendererPackage};var import_core_common5=require("@storybook/core-common"),import_find_up3=__toESM(require("find-up")),logger17=console,packagesMap={"@storybook/react":{webpack5:"@storybook/react-webpack5",vite:"@storybook/react-vite"},"@storybook/preact":{webpack5:"@storybook/preact-webpack5",vite:"@storybook/preact-vite"},"@storybook/server":{webpack5:"@storybook/server-webpack5"},"@storybook/ember":{webpack5:"@storybook/ember"},"@storybook/angular":{webpack5:"@storybook/angular"},"@storybook/vue":{webpack5:"@storybook/vue-webpack5",vite:"@storybook/vue-vite"},"@storybook/vue3":{webpack5:"@storybook/vue3-webpack5",vite:"@storybook/vue3-vite"},"@storybook/svelte":{webpack5:"@storybook/svelte-webpack5",vite:"@storybook/svelte-vite"},"@storybook/web-components":{webpack5:"@storybook/web-components-webpack5",vite:"@storybook/web-components-vite"},"@storybook/html":{webpack5:"@storybook/html-webpack5",vite:"@storybook/html-vite"}},communityFrameworks={vite:["storybook-framework-qwik","storybook-solidjs-vite"],webpack5:[]},viteConfigFiles2=["vite.config.js","vite.config.cjs","vite.config.mjs","vite.config.ts"],webpackConfigFiles2=["webpack.config.js","webpack.config.cjs","webpack.config.mjs","webpack.config.ts"],detectBuilderInfo=async({mainConfig,configDir,packageDependencies})=>{var _a;let builderOptions={},builderName,builderOrFrameworkName,{core={},framework}=mainConfig,{builder}=core;if(builder)typeof builder=="string"?builderOrFrameworkName=builder:(builderOrFrameworkName=builder.name,builderOptions=builder.options||{});else if(framework){let frameworkName=typeof framework=="string"?framework:framework.name;Object.keys(import_core_common5.frameworkPackages).includes(frameworkName)&&(builderOrFrameworkName=frameworkName,builderOptions=typeof framework=="object"?(_a=framework.options)==null?void 0:_a.builder:{})}if(!builderOrFrameworkName){let viteConfigFile=await(0,import_find_up3.default)(viteConfigFiles2,{cwd:configDir});if(viteConfigFile)logger17.info(`No builder or framework field, detected Storybook builder via: ${viteConfigFile}`),builderOrFrameworkName="vite";else{let webpackConfigFile=await(0,import_find_up3.default)(webpackConfigFiles2,{cwd:configDir});webpackConfigFile&&(logger17.info(`No builder or framework field, detected Storybook builder via: ${webpackConfigFile}`),builderOrFrameworkName="webpack5")}}return builderOrFrameworkName||(packageDependencies["@storybook/builder-vite"]||packageDependencies["storybook-builder-vite"]?builderOrFrameworkName="vite":(packageDependencies["@storybook/builder-webpack5"]||packageDependencies["@storybook/manager-webpack5"])&&(builderOrFrameworkName="webpack5")),builderOrFrameworkName!=null&&builderOrFrameworkName.includes("vite")||communityFrameworks.vite.includes(builderOrFrameworkName)?builderName="vite":(builderOrFrameworkName!=null&&builderOrFrameworkName.includes("webpack")||communityFrameworks.webpack5.includes(builderOrFrameworkName),builderName="webpack5"),{name:builderName,options:builderOptions}},getNextjsAddonOptions=addons=>{let nextjsAddon=addons==null?void 0:addons.find(addon=>typeof addon=="string"?addon==="storybook-addon-next":addon.name==="storybook-addon-next");return!nextjsAddon||typeof nextjsAddon=="string"?{}:nextjsAddon.options||{}};var logger18=console,nextJsConfigFiles=["next.config.js","next.config.cjs","next.config.mjs","next.config.ts"],newFrameworks={id:"new-frameworks",async check({rendererPackage:userDefinedRendererPackage,configDir:userDefinedConfigDir,packageManager}){var _a,_b,_c;let packageJson=await packageManager.retrievePackageJson(),{storybookVersion,mainConfig,mainConfigPath,configDir}=await getStorybookData({packageManager,configDir:userDefinedConfigDir});if(!import_semver16.default.gte(storybookVersion,"7.0.0"))return null;let frameworkPackage=typeof mainConfig.framework=="string"?mainConfig.framework:(_a=mainConfig.framework)==null?void 0:_a.name,hasFrameworkInMainConfig=!!frameworkPackage,rendererPackage=userDefinedRendererPackage;rendererPackage||(frameworkPackage&&Object.keys(import_core_common6.rendererPackages).includes(frameworkPackage)?rendererPackage=frameworkPackage:frameworkPackage&&Object.values(import_core_common6.rendererPackages).includes(frameworkPackage)?(rendererPackage=Object.keys(import_core_common6.rendererPackages).find(k=>import_core_common6.rendererPackages[k]===frameworkPackage),hasFrameworkInMainConfig=!1):rendererPackage=await detectRenderer(packageJson));let builderConfig=(_b=mainConfig.core)==null?void 0:_b.builder;if(!Object.keys(packagesMap).includes(rendererPackage))return null;let allDependencies=await packageManager.getAllDependencies(),builderInfo=await detectBuilderInfo({mainConfig,configDir,packageDependencies:allDependencies}),newFrameworkPackage=Object.keys(import_core_common6.frameworkPackages).find(pkg2=>pkg2===frameworkPackage);if(newFrameworkPackage||(newFrameworkPackage=packagesMap[rendererPackage]&&packagesMap[rendererPackage][builderInfo.name]),!newFrameworkPackage)return null;let renderer=import_core_common6.rendererPackages[rendererPackage],rendererOptions=mainConfig[`${renderer}Options`]||{},frameworkOptions=typeof mainConfig.framework=="string"?{}:(_c=mainConfig.framework)==null?void 0:_c.options,dependenciesToRemove=["@storybook/builder-webpack5","@storybook/manager-webpack5","@storybook/builder-webpack4","@storybook/manager-webpack4","@storybook/builder-vite","storybook-builder-vite"],addonsToRemove=[],addonOptions={},metaFramework;if(rendererPackage==="@storybook/react"&&allDependencies.next){let nextConfigFile=await(0,import_find_up4.default)(nextJsConfigFiles,{cwd:configDir}),nextAddonOptions=getNextjsAddonOptions(mainConfig.addons);(import_semver16.default.gte(import_semver16.default.coerce(allDependencies.next).version,"12.0.0")&&nextConfigFile||Object.keys(nextAddonOptions).length>0)&&(metaFramework="nextjs",(newFrameworkPackage==="@storybook/react-webpack5"||newFrameworkPackage==="@storybook/nextjs")&&(newFrameworkPackage="@storybook/nextjs",addonsToRemove=["storybook-addon-next","storybook-addon-next-router"].filter(dep=>{var _a2;return allDependencies[dep]||((_a2=mainConfig.addons)==null?void 0:_a2.find(addon=>typeof addon=="string"?dep===addon:dep===addon.name))}),addonOptions=nextAddonOptions,dependenciesToRemove.push("@storybook/react-webpack5","storybook-addon-next","storybook-addon-next-router")))}else rendererPackage==="@storybook/svelte"&&allDependencies["@sveltejs/kit"]&&import_semver16.default.gte(import_semver16.default.coerce(allDependencies["@sveltejs/kit"]).version,"1.0.0")&&(metaFramework="sveltekit",newFrameworkPackage==="@storybook/svelte-vite"&&(newFrameworkPackage="@storybook/sveltekit",rendererOptions={},dependenciesToRemove.push("@storybook/svelte-vite")));let dependenciesToAdd=[newFrameworkPackage].filter(dep=>!allDependencies[dep]).filter(Boolean);if(dependenciesToRemove=dependenciesToRemove.filter(dep=>allDependencies[dep]).filter(Boolean),hasFrameworkInMainConfig&&!builderConfig&&!Object.keys(rendererOptions).length&&!Object.keys(addonOptions).length&&!dependenciesToRemove.length&&!dependenciesToAdd.length)return null;if(allDependencies.vite&&import_semver16.default.lt(import_semver16.default.coerce(allDependencies.vite).version,"3.0.0"))throw new Error(import_ts_dedent23.default`
        ❌ Your project should be upgraded to use the framework package ${import_chalk15.default.bold(newFrameworkPackage)}, but we detected that you are using Vite ${import_chalk15.default.bold(allDependencies.vite)}, which is unsupported in ${import_chalk15.default.bold("Storybook 7.0")}. Please upgrade Vite to ${import_chalk15.default.bold("3.0.0 or higher")} and rerun this migration.
...
`)}
    ...
    `}};var import_chalk17=__toESM(require("chalk")),import_ts_dedent25=require("ts-dedent"),import_path13=require("path"),import_fs_extra11=__toESM(require("fs-extra")),import_globby2=__toESM(require("globby")),MDX1_STYLE_START=/<style>{`/g,MDX1_STYLE_END=/`}<\/style>/g,MDX1_COMMENT=/<!--(.+)-->/g,MDX1_CODEBLOCK=/(?:\n~~~(?:\n|.)*?\n~~~)|(?:\n```(?:\n|.)*?\n```)/g,fixMdxStyleTags=mdx=>mdx.replace(MDX1_STYLE_START,"<style>\n  {`").replace(MDX1_STYLE_END,"  `}\n</style>"),fixMdxComments=mdx=>{let codeblocks=mdx.matchAll(MDX1_CODEBLOCK);return mdx.split(MDX1_CODEBLOCK).map(v=>v.replace(MDX1_COMMENT,(original,group)=>`{/*${group}*/}`)).reduce((acc,item,i)=>{let next=codeblocks.next();return next.done?acc+item:acc+item+next.value[0]},"")},logger19=console,mdx1to2={id:"mdx1to2",async check(){let storiesMdxFiles=await(0,import_globby2.default)("./!(node_modules)**/*.(story|stories).mdx");return storiesMdxFiles.length?{storiesMdxFiles}:void 0},prompt({storiesMdxFiles}){return import_ts_dedent25.dedent`
      ...
      ${import_chalk17.default.cyan("https://storybook.js.org/migration-guides/7.0")}
    `},async run({result:{storiesMdxFiles},dryRun}){await Promise.all([...storiesMdxFiles.map(async fname=>{let contents=await import_fs_extra11.default.readFile(fname,"utf-8"),updated=fixMdxComments(fixMdxStyleTags(contents));updated===contents?logger19.info(`\u{1F197} Unmodified ${(0,import_path13.basename)(fname)}`):(logger19.info(`\u2705 Modified ${(0,import_path13.basename)(fname)}`),dryRun||await import_fs_extra11.default.writeFile(fname,updated))})])}};var import_chalk18=__toESM(require("chalk")),import_ts_dedent26=require("ts-dedent");var logger20=console,autodocsTrue={id:"autodocsTrue",async check({packageManager,configDir}){let{mainConfig}=await getStorybookData({packageManager,configDir}),{docs}=mainConfig,docsPageToAutodocsMapping={true:"tag",automatic:!0,false:!1};if(docs!=null&&docs.docsPage){let oldValue=docs==null?void 0:docs.docsPage.toString();if(!(oldValue in docsPageToAutodocsMapping))throw new Error(`Unexpected value for docs.docsPage: ${oldValue}`);return{value:docsPageToAutodocsMapping[oldValue]}}return(docs==null?void 0:docs.autodocs)===void 0?{value:!0}:null},prompt({value}){let autodocsFormatted=import_chalk18.default.cyan(`docs: { autodocs: ${JSON.stringify(value??!0)} }`),tagWarning=import_ts_dedent26.dedent`
    ...
    `;return value?import_ts_dedent26.dedent`
      ...
      More info: ${import_chalk18.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#autodocs-changes")}
...
`);return import_ts_dedent27.dedent`
    ...
    `},async run({packageManager,result:{additionalDependencies},dryRun}){dryRun||await packageManager.addDependencies({installAsDevDependencies:!0},additionalDependencies)}};var import_chalk20=__toESM(require("chalk")),import_ts_dedent28=__toESM(require("ts-dedent")),import_semver17=__toESM(require("semver"));var nodeJsRequirement={id:"nodejs-requirement",promptOnly:!0,async check({packageManager,configDir}){let{storybookVersion}=await getStorybookData({packageManager,configDir});if(!import_semver17.default.gte(storybookVersion,"7.0.0"))return null;let nodeVersion=process.version;return import_semver17.default.lt(nodeVersion,"16.0.0")?{nodeVersion}:null},prompt({nodeVersion}){return import_ts_dedent28.default`
    ...
    `}};var import_chalk21=__toESM(require("chalk")),import_ts_dedent29=__toESM(require("ts-dedent")),import_semver18=__toESM(require("semver")),import_core=require("@babel/core");var logger21=console,frameworksThatNeedBabelConfig=["@storybook/react-webpack5","@storybook/vue-webpack5","@storybook/vue3-webpack5","@storybook/html-webpack5","@storybook/web-components-webpack5"],missingBabelRc={id:"missing-babelrc",async check({configDir,packageManager}){let packageJson=await packageManager.retrievePackageJson(),{mainConfig,storybookVersion}=await getStorybookData({configDir,packageManager});if(!import_semver18.default.gte(storybookVersion,"7.0.0"))return null;let{framework,addons}=mainConfig,frameworkPackage=typeof framework=="string"?framework:framework.name,hasCraPreset=addons&&addons.find(addon=>addon==="@storybook/preset-create-react-app");if(frameworkPackage&&frameworksThatNeedBabelConfig.includes(frameworkPackage)&&!hasCraPreset){let config=await(0,import_core.loadPartialConfigAsync)({babelrc:!0,filename:"__fake__.js"});if(!config.config&&!config.babelrc&&!packageJson.babel)return{needsBabelRc:!0}}return null},prompt(){return import_ts_dedent29.default`
      ...
      ${import_chalk21.default.bold("Note:")} After installing the necessary presets, if it does not work in a monorepo, see the babel documentation for reference:
      ${import_chalk21.default.yellow("https://babeljs.io/docs")}

      Please see the migration guide for more information:
      ${import_chalk21.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#babel-mode-v7-exclusively")}
    `},async run(){logger21.info(),await generateStorybookBabelConfigInCWD()}};var import_ts_dedent30=require("ts-dedent"),import_semver19=__toESM(require("semver")),import_chalk22=__toESM(require("chalk")),import_prompts7=__toESM(require("prompts"));var angularBuilders={id:"angular-builders",async check({packageManager,configDir}){var _a;let packageJSON=await packageManager.retrievePackageJson();if(isNxProject(packageJSON))return null;let angularVersion=(await packageManager.getAllDependencies())["@angular/core"],angularCoerced=(_a=import_semver19.default.coerce(angularVersion))==null?void 0:_a.version;if(!angularCoerced)return null;if(import_semver19.default.lt(angularCoerced,"14.0.0"))throw new Error(import_ts_dedent30.dedent`
      ...
      `);let angularJSON=new AngularJSON,{hasStorybookBuilder}=angularJSON;if(hasStorybookBuilder||!angularJSON.rootProject&&Object.keys(angularJSON.projects).length>1)return null;let{mainConfig}=await getStorybookData({configDir,packageManager});return{mainConfig,packageManager}},prompt(){return import_ts_dedent30.dedent`
...
`)}
      ...
      ${import_chalk23.default.yellow("https://github.com/storybookjs/storybook/issues/20529")}
    `}};var import_ts_dedent32=require("ts-dedent"),import_semver21=__toESM(require("semver")),import_chalk24=__toESM(require("chalk"));var angularBuildersMultiproject={id:"angular-builders-multiproject",promptOnly:!0,async check({packageManager}){var _a;let packageJSON=await packageManager.retrievePackageJson();if(isNxProject(packageJSON))return null;let angularVersion=(await packageManager.getAllDependencies())["@angular/core"],angularCoerced=(_a=import_semver21.default.coerce(angularVersion))==null?void 0:_a.version;if(!angularCoerced||import_semver21.default.lt(angularCoerced,"14.0.0"))return null;let angularJSON=new AngularJSON,{hasStorybookBuilder}=angularJSON;return hasStorybookBuilder||angularJSON.rootProject||Object.keys(angularJSON.projects).length===1?null:{}},prompt(){return import_ts_dedent32.dedent`
    In Storybook 6.4 we have deprecated calling Storybook directly (npm run storybook) for Angular. In Storybook 7.0, we've removed it entirely. Instead you have to set up the Storybook builder in your ${import_chalk24.default.yellow("angular.json")} and execute ${import_chalk24.default.yellow("ng run <your-project>:storybook")} to start Storybook. 
    
    ❌ Your Angular workspace uses multiple projects defined in the ${import_chalk24.default.yellow("angular.json")} file and we were not able to detect a root project. Therefore we are not able to automigrate to use Angular Storybook builder. Instead, please visit ${import_chalk24.default.yellow("https://github.com/storybookjs/storybook/tree/next/code/frameworks/angular")} to do the migration manually.
    `}};var allFixes=[nodeJsRequirement,cra5,webpack5,angular12,vue3,eslintPlugin,builderVite,sbBinary,sbScripts,newFrameworks,incompatibleAddons,removedGlobalClientAPIs,mdx1to2,mdxgfm,autodocsTrue,addReact,missingBabelRc,angularBuildersMultiproject,angularBuilders];var import_os2=require("os"),ansiRegex=({onlyFirst=!1}={})=>{let pattern=["[\\u001B\\u009B][[\\]()#;?]*(?:(?:(?:(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]+)*|[a-zA-Z\\d]+(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]*)*)?\\u0007)","(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PR-TZcf-ntqry=><~]))"].join("|");return new RegExp(pattern,onlyFirst?void 0:"g")},cleanLog=str=>str.replace(ansiRegex(),"").replace(/╮│/g,`\u256E
\u2502`).replace(/││/g,`\u2502
\u2502`).replace(/│╰/g,`\u2502
\u2570`).replace(/⚠️ {2}failed to check/g,`${import_os2.EOL}\u26A0\uFE0F  failed to check`);var import_chalk25=__toESM(require("chalk")),import_boxen=__toESM(require("boxen")),import_core_common8=require("@storybook/core-common"),import_ts_dedent33=__toESM(require("ts-dedent"));var import_semver22=__toESM(require("semver"));function hasMultipleVersions(versions){return versions.find(v=>{let major=import_semver22.default.major(v);if(major===0){let minor=import_semver22.default.minor(v);if(minor===0){let patch=import_semver22.default.patch(v);return versions.some(v2=>import_semver22.default.patch(v2)!==patch)}return versions.some(v2=>import_semver22.default.minor(v2)!==minor)}return versions.some(v2=>import_semver22.default.major(v2)!==major)})}var messageDivider=`
...
`);return import_ts_dedent35.default`
    ...
    To learn more about this change, see: ${import_chalk27.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
  `},async run({dryRun,mainConfigPath,result:{nextStoriesEntries}}){logger23.info(import_ts_dedent35.default`✅ Setting 'stories' config:
...
`)}
            `.trim(),{borderStyle:"round",padding:1,borderColor:"#F1618C"})),process.exit(1)),choices.length===1?[selectedTemplate]=choices:(logger27.info((0,import_boxen3.default)(import_ts_dedent36.dedent`
...
yarn storybook`):`Recreate your setup, then ${import_chalk28.default.yellow("npx storybook@latest init")}`;logger27.info((0,import_boxen3.default)(import_ts_dedent36.dedent`
